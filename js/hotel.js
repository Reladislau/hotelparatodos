...
function inicio() {
    ...
    menu = parseInt(prompt("O Que você deseja? Escolha a opção digitando abaixo \n1- Reservar Quartos \n2- Cadastro \n3- Cadastrar e Pesquisar \n4- Eventos \n5- Buffet \n6- Auditório \n7- Restaurante \n8- Abastecer Carro \n9-Manut. Ar Condicionado \n10- Sair"));
...
function loginFuncionario() {
    ...
    while (senha !== senha) {
        ...
        senha = prompt("Digite Novamente a sua senha: ");
    }
...
function sair() {
    confirma = confirm("Você deseja sair ?");
    if (confirma) {
...
function cadastroComum() {
    valorPadrao = prompt("Qual o valor da diária?");
    if (valorPadrao === null) {
        inicio();
        return;
    }
    ...
    while (true) {
        ...
        if (nome.toUpperCase() === "PARE") {
            ...
            if (isNaN(idadeUsuario) || idadeUsuario === "" || idadeUsuario <= 0) {
                ...
                alert(nome + " Cadastro feito com sucesso. " + nome + " Possui Gratuídade.");
                gratuidade++;

            } else if (idadeUsuario >= 60) {
                alert(nome + " Cadastro feito com sucesso. " + nome + " Paga meia.");
                somaTotal += valorPadrao / 2;
                meiaDiaria++;

            } else {
                alert(nome + " Cadastro feito com sucesso. ");
                somaTotal += valorPadrao;
            }
        }
    }
    alert(`${nome}, o valor total é de R$${somaTotal}; ${gratuidade} grauidade(s); ${meiaDiaria} meias(s)`);
    inicio();
}
...
function abastecerCarro() {
    while (true) {
        alcoolPosto = parseFloat(prompt("Qual o valor do álcool no posto Wayne Oil?"));
        if (!isNaN(alcoolPosto) && alcoolPosto > 0) {
            break;
        } else {
            alert('Valor Inválido.');
        }
    ...
    while (true) {
        gasolinaPosto = parseFloat(prompt("Qual o valor da gasolina no posto Wayne Oil?"));
        if (!isNaN(gasolinaPosto) && gasolinaPosto > 0) {
            break;
        } else {
            alert('Valor Inválido.');
        }
    ...
    while (true) {
        alcoolPosto = parseFloat(prompt("Qual o valor do álcool no posto Stark Petrol?"));
        if (!isNaN(alcoolPosto) && alcoolPosto > 0) {
            break;
        } else {
            alert('Valor Inválido.');
    ...
    while (true) {
        gasolinaPosto = parseFloat(prompt("Qual o valor da gasolina no posto Stark Petrol?"));
        if (!isNaN(gasolinaPosto) && gasolinaPosto > 0) {
            break;
        } else {
            alert('Valor Inválido.');
    ...
    abastecerCarro2(alcoolPosto, gasolinaPosto, alcoolPosto, gasolinaPosto);
}

function abastecerCarro2(alcoolPosto, gasolinaPosto, alcoolPosto, gasolinaPosto) {
    var litrosAbastecidos = 42;
    var totalalcoolPosto = alcoolPosto * litrosAbastecidos;
    var totalgasolinaPosto = gasolinaPosto * litrosAbastecidos;
    var totalalcoolPosto = alcoolPosto * litrosAbastecidos;
    var totalgasolinaPosto = gasolinaPosto * litrosAbastecidos;

    let combustivelPosto1, precoTotalWayne;
    if (totalalcoolPosto / totalgasolinaPosto < 0.7) {
        combustivelPosto1 = "álcool";
        precoTotalWayne = totalalcoolPosto;
    } else {
        combustivelPosto1 = "gasolina";
        precoTotalWayne = totalgasolinaPosto;
    }

    let combustivelPosto2, precoTotalStark;
    if (totalalcoolPosto / totalgasolinaPosto < 0.7) {
        combustivelPosto2 = "álcool";
        precoTotalStark = totalalcoolPosto;
    } else {
        combustivelPosto2 = "gasolina";
        precoTotalStark = totalgasolinaPosto;
    }

    let postoMaisBarato;
    if (precoTotalWayne < precoTotalStark) {
        postoMaisBarato = "Wayne Oil";
        alert(`É mais barato abastecer com ${combustivelPosto1} no posto ${postoMaisBarato}.`);
        menuInicio();
    } else {
        postoMaisBarato = "Stark Petrol";
        alert(`É mais barato abastecer com ${combustivelPosto2} no posto ${postoMaisBarato}.`);
        menuInicio();
    }
}
...
function arCondicionado() {
    ...
    while (true) {
        ...
        alert(`O serviço da empresa ${nomeEmpresa} terá o cusrto de R$ ${valorTotal.toFixed(2)}`);

        var continuar = prompt('Deseja refazer? S/N').toUpperCase();
        if (continuar !== 'S') {
            break;
        }
    }
